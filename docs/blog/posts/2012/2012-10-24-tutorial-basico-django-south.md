...
[South](http://south.aeracode.org/) es una app de [django](https://www.djangoproject.com/) que permite modificar la estructura de la base de datos de una aplicación django cuando cambiamos el modelo _(models.py)_.

El comando [syncdb](https://docs.djangoproject.com/en/dev/ref/django-admin/#django-admin-syncdb) sólo crea nuevas tablas, pero no modifica tablas existentes, así que si en el modelo de una aplicación renombramos un campo de una tabla existente syncdb no realizará ese cambio en la base de datos. A este tipo de cambios en la base de datos se les denomina _«migración del esquema»_ y es de lo que se encarga South.
