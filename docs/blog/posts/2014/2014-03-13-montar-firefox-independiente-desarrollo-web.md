...
5. El paso anterior habrá modificado el fichero profiles.ini, añadiendo el nuevo. Pero lo que nosotros queremos es tener el _modo desarrollo_ lo más aislado posible así que restauramos la copia que hicimos anteriormente
6. Creamos un miniscript en nuestro PATH o donde queramos que ejecute nuestro firefox de desarrollo y le pase [el parámetro](http://kb.mozillazine.org/Command_line_arguments) `-profile` y el path al perfil de desarrollo que creamos. Yo cree un script _ffdev_ en /usr/local/bin con el siguiente contenido  
